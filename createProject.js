...
export default function initializeProject() {
  ...
  inquirer.prompt(questionsToCreateProject).then(async (answers) => {
    ...
    // Génération du front-end (ex: Angular)
...
/**************************** backend!!! ***************************/
    ...
    //console.log(purpleText, `Creating Spring Boot project ${projectName}...`);
    //execSync(`npx spring init --dependencies=mysql,jpa,web,devtools --groupId=${packageName} --artifactId=${projectName} --name=${projectName} --package-name=${packageName} --description="New Spring Boot Project" --language=java --build=gradle ${projectName}`, { stdio: 'inherit' });
    //console.log(`Spring Boot project "${projectName}" created successfully.`);
